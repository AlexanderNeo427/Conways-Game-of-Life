...
void FlockingSim::Init()
{
	for (int i = 0; i < NUM_BOIDS; ++i)
	{
		...
		glm::vec2 vel = glm::circularRand(1.f);
...
void FlockingSim::Update(float deltaTime)
{
	...
	if (m_pge->GetKey(olc::P).bHeld) m_separation = glm::min(15.f, m_separation + deltaTime);

	if (m_pge->GetKey(olc::K).bHeld) m_alignment = glm::max(0.01f, m_alignment - deltaTime);
	if (m_pge->GetKey(olc::L).bHeld) m_alignment = glm::min(15.f, m_alignment + deltaTime);

	if (m_pge->GetKey(olc::N).bHeld) m_cohesion = glm::max(0.01f, m_cohesion - deltaTime);
	if (m_pge->GetKey(olc::M).bHeld) m_cohesion = glm::min(15.f, m_cohesion + deltaTime);
